...
new Vue({
  ...
  async created() {
    ...
    if (this.id) {
      request({ url: '/api/Bangwen/bangwenDetail', method: 'post', data: { bangwen_id: this.id }, }).then((res) => {
        if (res.code == 200) {
          this.formData = {
            type: res.data.type,// 1教课，2学课
            title: res.data.title,// 标题
            b_id: res.data.b_id,// 服务分类id
            total_money: res.data.total_money,// 总金额
            is_trustee: res.data.is_trustee,// 1托管，2不托管
            pay_num_type: res.data.pay_num_type,// 1一次性付清,2多次付清
            pay_num: res.data.pay_num,// 付款次数
            more_type: res.data.more_type,// 1均分，2自定义金额
            win_type: res.data.win_type,// 1单人中榜，2多人中榜
            win_num: res.data.win_num,// 中榜人数
            task_start_time: res.data.task_start_time,// 任务开始时间
            task_end_time: res.data.task_end_time,// 任务结束时间
            signup_start_time: res.data.signup_start_time,// 报名开始时间
            signup_end_time: res.data.signup_end_time,// 报名结束时间
            detail: res.data.detail,// 榜文详情
            image: res.data.image,// 封面（路径如：/uploads/20211216/6d39ccf1e51e8e21a0ba946de64cb8f0.jpg）
            files: res.data.files,// 文件（路径如：/uploads/20211216/6d39ccf1e51e8e21a0ba946de64cb8f0.jpg）
            mobile: res.data.mobile,// 手机号
            qq: res.data.qq,// qq号
            email: res.data.email,// 邮箱号
            moneys: '',// 如果是多次付清的自定义金额，如10,5,5
            bangwen_id: this.id,// 榜文id
          }
          this.totalMoney = res.data.total_money
          res.data.moneys.map((item, k) => {
            item.num = item.money
            item.id = k + 1
          })
          this.ordeList = res.data.moneys
          let arr1 = []
          for (let i in res.data.moneys) {
  ...
  methods: {
    ...
    async onBtnClick() {
      ...
      if (this.formData.qq) {
        var reg_tel = /^[1-9][0-9]{4,10}$/gim
        if (!reg_tel.test(this.formData.qq)) return layer.msg('请输入正确的QQ')
      }
      if (this.formData.email) {
        var reg_tel = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/
        if (!reg_tel.test(this.formData.email)) return layer.msg('请输入正确的邮箱')
      }
      ...
      if (this.id) {
        // 修改
        request({ url: '/api/Bangwen/bangwenDetail', method: 'post', data: this.formData }).then((res) => {
          if (res.code == 200) {
        ...
        // 添加
        request({ url: '/api/Bangwen/pushBangwen', method: 'post', data: this.formData }).then((res) => {
          if (res.code == 200) {
            location.href = './noticeList.html'
          } else {
            layer.msg(res.msg)
