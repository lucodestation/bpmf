...
new Vue({
  ...
  methods: {
    ...
    // 列表数据
    onpushBangwenOrder() {
      request({ url: '/api/vip/orderList', method: 'POST', data: { page: 1, pagenum: 10, search: this.search, status: this.navId, type: this.type, start_time: this.start_time, end_time: this.end_time } }).then((res) => {
        if (res.code == 200) {
