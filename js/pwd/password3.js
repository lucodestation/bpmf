    ...
    methods: {
        onAddClick: function () {
            ...
            if (this.pwd.length < 8 || this.pwd.length > 20)
                return layer.msg('密码长度8~20个字符');
            if (!this.pwd2)
                return layer.msg('请再次输入密码');
            if (this.pwd2.length < 8 || this.pwd2.length > 20)
                return layer.msg('密码长度8~20个字符');
            if (this.pwd !== this.pwd2)
                return layer.msg('2次输入密码不一样，请重新输入');
            encrypt.setPublicKey(publiukey);
            // 加密
            var pwd = encrypt.encrypt(this.pwd); //需要加密的内容
            var pwd2 = encrypt.encrypt(this.pwd2); //需要加密的内容
            request({ url: '/api/Login/forgetPwd', method: 'post', data: { mobile: this.mobile, code: this.code, pwd: pwd, pwd2: pwd2 } }).then(function (res) {
                if (res.code == 200) {
